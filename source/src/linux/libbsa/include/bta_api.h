...
#define BTA_DM_BLE_AD_BIT_SERVICE_32SOL   BTM_BLE_AD_BIT_SERVICE_32SOL
...
#define BTA_DM_BLE_AD_BIT_MESH_BEACON     BTM_BLE_AD_BIT_MESH_BEACON      /*Mesh Beacon*/
#define BTA_DM_BLE_AD_BIT_MESH_MESSAGE    BTM_BLE_AD_BIT_MESH_MESSAGE     /*Mesh Message*/
#define BTA_DM_BLE_AD_BIT_MESH_PBADV      BTM_BLE_AD_BIT_MESH_PBADV       /*Mesh PB-ADV*/

/* ++Mesh definition ++*/
#define BTA_BLE_AD_DATA_LEN                  31

#define ADV_UNACKED_REPEAT_COUNT             4

//TBD - Temporarily fixed the AD TYPE+AD LEN. Need to dynamic to set.
#define AD_TYPE_LEN                          2

#define MESH_TRANSACTION_START       0
#define MESH_TRANSACTION_ACK         1
#define MESH_TRANSACTION_CONTINUE    2
#define MESH_CONTROL                 3

#define MESH_LINK_OPEN      0x00
#define MESH_LINK_ACK       0x01
#define MESH_LINK_CLOSE     0x02

#define DEVICE_UUID_LEN     16
#define PBADV_LINK_ID_LEN   4

#define PBADV_PACKET_OVERHEAD               (4 /* Link ID */ + 1 /* Transaction number */)
#define TRANSACTION_START_OVERHEAD          (1 /* SegN+GPCF */ + 2 /* Total len */ + 1 /* FCS */)
#define TRANSACTION_ACK_LEN                 (1 /* SegN+GPCF */)
#define TRANSACTION_CONTINUATION_OVERHEAD   (1 /* SegN+GPCF */)
#define LINK_OPEN_LEN                       (1 /* Control ID+GPCF */ + 16)//NRF_MESH_UUID_SIZE)
#define LINK_ACK_LEN                        (1 /* Control ID+GPCF */)
#define LINK_CLOSE_LEN                      (1 /* Control ID+GPCF */ + 1 /* Reason */)

#define BLE_AD_DATA_OVERHEAD 1

/* Max length of TRANSACTION_START = 31-5-4 = 22 -2 =20*/
#define TRANSACTION_START_MAXLEN  (BTA_BLE_AD_DATA_LEN -\
                                   (AD_TYPE_LEN + \
                                    PBADV_PACKET_OVERHEAD + \
                                    TRANSACTION_START_OVERHEAD))


/* Max length of TRANSACTION_CONTINUATION = 31-5-1 = 25 -2 = 23*/
#define TRANSACTION_CONTINUATION_MAXLEN (BTA_BLE_AD_DATA_LEN -\
                                         (AD_TYPE_LEN + \
                                          PBADV_PACKET_OVERHEAD + \
                                          TRANSACTION_CONTINUATION_OVERHEAD))

#define UNPROVISIONED_DEVICE_BEACON 0x00
#define SECURE_NETWORK_BEACON 0x01

#define PB_ADV_TRANSACTION_NUMBER_PROVISIONER_START  (0x00)
#define PB_ADV_TRANSACTION_NUMBER_PROVISIONEE_START  (0x80)

#define PROV_TYPE_INVITE     0x00
#define PROV_TYPE_CAPS       0x01
#define PROV_TYPE_START      0x02
#define PROV_TYPE_PUB_KEY    0x03
#define PROV_TYPE_INP_CMPLT  0x04
...
typedef struct
{
    tBT_UUID    service_uuid[MAX_SERVICE_DATA_NUM];
    UINT8       len[MAX_SERVICE_DATA_NUM];
    UINT8      *p_val[MAX_SERVICE_DATA_NUM];
...
typedef union
{
    struct
    {
        UINT8 uuid[DEVICE_UUID_LEN];
    } link_open;
    struct
    {
        UINT8 reason; /* Close Reason*/
    } link_close;
} tBTA_GENERIC_PROC_CONTROL;
...
typedef union
{
    struct
    {
        ...
        UINT8   fcs;                                /* Frame Check Sequence */
        UINT8   payload[TRANSACTION_START_MAXLEN];  /* Packet payload */
    } tran_start;

    struct
    {
        UINT8  payload[TRANSACTION_CONTINUATION_MAXLEN];  /* Packet payload */
    } tran_cont;
} tBTA_GENERIC_TRANSACTION;
...
typedef struct
{
    ...
    union
    {
        tBTA_GENERIC_PROC_CONTROL  control;     /* Control PDU. */
        tBTA_GENERIC_TRANSACTION   transaction; /* Transaction PDU. */
    } payload;
} tBTA_GENERIC_PROV_PDU;
...
typedef struct
{
    ...
    UINT32              linkID;
    UINT8               trans_num;
    tBTA_GENERIC_PROV_PDU   generic_prov_pdu;
}tBTA_BLE_MESH_PBADV;
...
#if (defined(BLE_INCLUDED_MESH) && BLE_INCLUDED_MESH == TRUE)
    tBTA_BLE_MESH_BEACON    *p_mesh_beacon;     /* mesh beacon */
    tBTA_BLE_MESH_NET       *p_mesh_net;        /* mesh net message */
    tBTA_BLE_MESH_PBADV     *p_mesh_pbadv;      /* mesh pbadv */
#endif
...
#if (defined(BLE_INCLUDED_MESH) && BLE_INCLUDED_MESH == TRUE)
typedef void (tBTA_SET_ADV_DATA_CMPL_CBACK) (tBTA_BLE_ADV_DATA* adv_data, tBTA_STATUS status);
#else
...
typedef struct
{
    ...
    tBTA_DM_TX_POWER_VALUE tx_power_value;
...
typedef union
{
    ...
    tBTA_DM_VSC_CMPL      vsc_cmpl;       /* vsc event completed result */
