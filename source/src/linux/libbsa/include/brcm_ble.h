...
/* Mesh multi advs(instance 1,2,3,4) maybe all call param/data/enable.
*  And then 1 instance would enqueue 3 elements.
*/
#define BTM_BLE_MULTI_ADV_ENQ_MAX  ((BTM_BLE_MULTI_ADV_MAX) * 3)
typedef struct
{
#if (defined(BLE_INCLUDED_MESH) && BLE_INCLUDED_MESH == TRUE)
    UINT8   sub_code[BTM_BLE_MULTI_ADV_ENQ_MAX];
    UINT8   inst_id[BTM_BLE_MULTI_ADV_ENQ_MAX];
#else
