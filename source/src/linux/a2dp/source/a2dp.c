...
int app_avk_rc_volume_up(tAPP_AVK_CONNECTION *conn)
{
    if(conn == NULL)
    {
        APP_ERROR0("Connection handle is NULL!");
        return -1;
    }

    if (!conn->is_rc_open)
    {
        APP_ERROR1("Unable to send AVRC command, is rc open %d", conn->is_rc_open);
        return -1;
    }

    /* If abs vol is not supported, send vol pass thru command */
    if (conn->m_bAbsVolumeSupported == FALSE)
    {
        app_avk_rc_send_command(BSA_AVK_STATE_PRESS, AVRC_ID_VOL_UP, conn->rc_handle);
        GKI_delay(300);
        app_avk_rc_send_command(BSA_AVK_STATE_RELEASE, AVRC_ID_VOL_UP, conn->rc_handle);
    }
    else
    {
        /* increase vol by 10% */
        ...
        app_avk_reg_notfn_rsp(app_avk_cb.volume, conn->rc_handle, conn->volChangeLabel,
            AVRC_EVT_VOLUME_CHANGE, BSA_AVK_RSP_CHANGED);
    }

    return 0;
}

int app_avk_rc_volume_down(tAPP_AVK_CONNECTION *conn)
{
    if(conn == NULL)
    {
        APP_ERROR0("Connection handle is NULL!");
    ...
    if (conn->m_bAbsVolumeSupported == FALSE)
    {
        app_avk_rc_send_command(BSA_AVK_STATE_PRESS, AVRC_ID_VOL_DOWN, conn->rc_handle);
        GKI_delay(300);
        app_avk_rc_send_command(BSA_AVK_STATE_RELEASE, AVRC_ID_VOL_DOWN, conn->rc_handle);
    }
    else
    {
        /* increase vol by 10% */
