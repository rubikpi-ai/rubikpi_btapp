...
enum
{
    ...
    APP_AVK_MENU_INC_VOL,
    APP_AVK_MENU_DEC_VOL
};


/*******************************************************************************
 **
 ** Function         a2dp_menu_display
 **
...
{
    APP_INFO0("Bluetooth Application A2dp menu:");
    ...
    APP_INFO1("\t%d  => AVK Inc Volume", APP_AVK_MENU_INC_VOL);
    APP_INFO1("\t%d  => AVK Dec Volume", APP_AVK_MENU_DEC_VOL);
...
int main(int argc, char **argv)
{
    ...
    do
    {
        ...
        case APP_AVK_MENU_INC_VOL:
            printf("Choose connection index\n");
            app_avk_display_connections();
            connection_index = app_get_choice("\n");
            connection = app_avk_find_connection_by_index(connection_index);
            if(connection)
                app_avk_rc_volume_up(connection);
            else
                printf("Unknown choice:%d\n", connection_index);
            break;

        case APP_AVK_MENU_DEC_VOL:
            printf("Choose connection index\n");
            app_avk_display_connections();
            connection_index = app_get_choice("\n");
            connection = app_avk_find_connection_by_index(connection_index);
            if(connection)
                app_avk_rc_volume_down(connection);
            else
                printf("Unknown choice:%d\n", connection_index);
            break;
...
int a2dp_menu_handle(void)
{
    ...
    app_mgt_custom_cback_update(app_avk_mgt_callback);
    /* Init XML state machine */
    app_xml_init();
