...
int app_ble_read_phy(UINT16 handle)
{
    ...
    hci_cmd.opcode = HCI_BLE_READ_PHY;
    hci_cmd.length = HCIC_PARAM_SIZE_BLE_READ_PHY;
    hci_cmd.no_opcode_swap = TRUE;
    p_data = hci_cmd.data;

    UINT16_TO_STREAM(p_data, handle);
...
int app_ble_set_phy(UINT16 handle, UINT8 tx_phys, UINT8 rx_phys, UINT16 phy_options)
{
    ...
    hci_cmd.opcode = HCI_BLE_SET_PHY;
    hci_cmd.length = HCIC_PARAM_SIZE_BLE_SET_PHY;
    hci_cmd.no_opcode_swap = TRUE;
    p_data = hci_cmd.data;

    UINT16_TO_STREAM(p_data, handle);
    UINT8_TO_STREAM(p_data, all_phys);
    UINT8_TO_STREAM(p_data, tx_phys);
    UINT8_TO_STREAM(p_data, rx_phys);
    UINT16_TO_STREAM(p_data, phy_options);
...
int app_ble_set_default_phy(UINT8 all_phys, UINT8 tx_phys, UINT8 rx_phys)
{
    ...
    hci_cmd.opcode = HCI_BLE_SET_DEFAULT_PHY;
    hci_cmd.length = HCIC_PARAM_SIZE_BLE_SET_DEFAULT_PHY;
    hci_cmd.no_opcode_swap = TRUE;
    p_data = hci_cmd.data;
