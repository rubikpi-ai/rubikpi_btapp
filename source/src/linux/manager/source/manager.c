...
/* Default local Name */
#define APP_DEFAULT_BT_NAME             "BSA_HOGP"
...
#define APP_DEFAULT_ROOT_PATH           "/usr/src/rubikpi-btapp/pictures"
...
void app_mgr_security_callback(tBSA_SEC_EVT event, tBSA_SEC_MSG *p_data)
{
    ...
    case BSA_SEC_SP_CFM_REQ_EVT: /* Simple Pairing confirm request */
        ...
        APP_DEBUG0("    You must accept or refuse using menu(15---Accept,16---Refuse)\n");
   ...
   case BSA_SEC_SP_RMT_OOB_EVT: /* Simple Pairing Remote OOB Data request. */
    ...
    case BSA_SEC_RSSI_EVT:
        ...
        {
            if (!bdcmp(p_data->sig_strength.bd_addr, app_xml_remote_devices_db[i].bd_addr)){

                APP_INFO1("\tDevice name: %s", app_xml_remote_devices_db[i].name);
                break;
            }
        }
    ...
    case BSA_SEC_TX_POWER_EVT:
        APP_INFO1("BSA_SEC_TXPOWER_EVT received for BdAddr: %02x:%02x:%02x:%02x:%02x:%02x",
                p_data->sig_strength.bd_addr[0], p_data->sig_strength.bd_addr[1],
                p_data->sig_strength.bd_addr[2], p_data->sig_strength.bd_addr[3],
                p_data->sig_strength.bd_addr[4], p_data->sig_strength.bd_addr[5]);
        if (p_data->sig_strength.mask & BSA_SEC_SIG_STRENGTH_TX_POWER)
        {
            if (p_data->sig_strength.link_type == BSA_TRANSPORT_BR_EDR)
            {
                /* For BasicRate & EnhancedDataRate links, the Tx Power is an attenuation (dB) */
                APP_INFO1("\tTX POWER: %d (dB)", p_data->sig_strength.tx_power_value);
            }
            else
            {
                /* For Low Energy links, the  Tx Power is a strength (dBm) */
                APP_INFO1("\tTX POWER: %d (dBm)", p_data->sig_strength.tx_power_value);
            }
        }
        break;
...
int app_mgr_read_version(void)
{
    ...
    APP_INFO1("BSA Server Version Information:%s", bsa_read_version.bsa_server_version_info);
    return 0;
}
