...
enum
{
    APP_MGR_MENU_INIT_BT          = 2,
    APP_MGR_MENU_ABORT_DISC,
...
//Global MGR initialization flag
extern int g_MGR_Init;
int g_mgr_config = 0;
/*
 * Local functions
 */
...
{
    APP_INFO0("Bluetooth Application Manager menu:");
#ifndef DEMO_MODE
    APP_INFO1("\t%d => Init", APP_MGR_MENU_INIT_BT);
#endif
    ...
    APP_INFO1("\t%d => Tx Power Measurement", APP_MGR_MENU_MONITOR_TXPOWER);
...
int main(int argc, char **argv)
{
    ...
    do
    {
        ...
        case APP_MGR_MENU_MONITOR_TXPOWER:
            choice = app_get_choice("Enable/Disable Tx Power Monitoring (1 = enable, 0 = disable)");
            if(choice > 0)
            {
                choice = app_get_choice("Enter Tx Power measurement period in seconds");
                APP_INFO0("Enabling Tx Power Monitoring");
                app_dm_monitor_txpower(TRUE,choice);
            }
            else
            {
                APP_INFO0("Disabling Tx Power Monitoring");
                app_dm_monitor_txpower(FALSE,0);
...
int manager_menu_handle(void){
    ...
    do
    {
        ...
        case HELP_MENU:
            manager_menu_display();
            break;
        case APP_MGR_MENU_INIT_BT:
                /* Init App manager */
            if (!g_MGR_Init)
            {
                app_mgt_init();
                if (app_mgt_open(BTAPP_UIPC_PATH, app_mgr_mgt_callback) < 0)
                {
                    APP_ERROR0("Unable to connect to server");
                    return -1;
                }
        ...
        default:
            APP_ERROR1("Unknown choice:%d",__func__, choice);
            manager_menu_display();
            break;
        }
    } while (choice != MENU_QUIT);      /* While user don't exit application */
