...
int app_dm_set_ble_adv_data(tBSA_DM_BLE_ADV_CONFIG *p_data)
{
    ...
    bt_config.adv_config.tx_power = p_data->tx_power;
    ...
    APP_ERROR1("sizeof(uuid)=%d, sizeof(uuid[0])=%d, sizeof(value[0])=%d, sizeof(value)=%d",
        sizeof(p_data->service_data_uuid), sizeof(p_data->service_data_uuid[0]), sizeof(p_data->service_data_val[0]), sizeof(p_data->service_data_val));
    memcpy(bt_config.adv_config.service_data_len, p_data->service_data_len, sizeof(bt_config.adv_config.service_data_len));
    memcpy(bt_config.adv_config.service_data_uuid, p_data->service_data_uuid, sizeof(bt_config.adv_config.service_data_uuid));
    memcpy(&bt_config.adv_config.service_data_val[0][0], &p_data->service_data_val[0][0],  sizeof(bt_config.adv_config.service_data_val));
    for (i=0; i<BSA_DM_BLE_AD_UUID_MAX; i++)
    {
        APP_DEBUG1("%d service_data_len=%d, uuid_len=%d, uuid=0x%02x:%02x, data=0x%02x:%02x:%02x", i, bt_config.adv_config.service_data_len[i],
            bt_config.adv_config.service_data_uuid[i].len, bt_config.adv_config.service_data_uuid[i].uu.uuid128[0],
            bt_config.adv_config.service_data_uuid[i].uu.uuid128[1], bt_config.adv_config.service_data_val[i][0],
            bt_config.adv_config.service_data_val[i][1], bt_config.adv_config.service_data_val[i][2]);
    }

    memcpy(&bt_config.adv_config.proprietary, &p_data->proprietary, sizeof(tBSA_DM_BLE_PROPRIETARY));
...
/*******************************************************************************
 **
 ** Function        app_dm_monitor_txpower
 **
 ** Description     Enables/Disables periodic monitoring of the Tx Power
 **                 for all connected devices
 **
 ** Parameters      enable : 1 - Enable, 0 - Disable
 **                 period : Measurement period in seconds
 ...
 **
 *******************************************************************************/
int app_dm_monitor_txpower(BOOLEAN enable, UINT16 period)
{
    tBSA_DM_SET_CONFIG bt_config;
    tBSA_STATUS bsa_status;

    APP_DEBUG1("%s Tx Power Monitoring\n", (enable)?"Enabling":"Disabling" );
    ...
    /* Set visibility configuration */
    bt_config.config_mask = BSA_DM_CONFIG_MONITOR_TXPOWER_MASK;

    /* Turn On or Off Tx Power Monitoring */
    bt_config.monitor_txpower_param.enable = enable;

    if ( bt_config.monitor_txpower_param.enable )
    {
        bt_config.monitor_txpower_param.period = period;
    }
