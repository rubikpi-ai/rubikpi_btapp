...
/*******************************************************************************
 **
 ** Function        app_tm_vsc_tx_test
 **
 ** Description     This function sends a Tx_TEST VSC
 **
 ** Parameters      None
 **
 ** Returns         status: 0 if success / -1 otherwise
 **
 *******************************************************************************/
int app_tm_vsc_tx_test()
{
    tBSA_TM_VSC bsa_vsc;
    tBSA_STATUS bsa_status;
    UINT8 *p_data;
    UINT8 priority;
    tBSA_TM_TX_TEST_CMD tx_test_cmd;

    APP_INFO0("app_tm_vsc_tx_test");

    isTxTest = TRUE;

    memset(&tx_test_cmd,0,sizeof(tBSA_TM_TX_TEST_CMD));

    APP_INFO0("BR/EDR Tx test:\n");
    APP_INFO0("Enter the BD address of Local Tx device(AA.BB.CC.DD.EE.FF):");

    if (scanf("%hhx.%hhx.%hhx.%hhx.%hhx.%hhx",
            &tx_test_cmd.bd_addr[0], &tx_test_cmd.bd_addr[1],
            &tx_test_cmd.bd_addr[2], &tx_test_cmd.bd_addr[3],
            &tx_test_cmd.bd_addr[4], &tx_test_cmd.bd_addr[5]) != 6)
    {
        APP_ERROR0("BD address not entered correctly");
        return;
    }
    ...
    APP_INFO0("Sets the hopping mode:\n");
    APP_INFO0("    0: 79 channel\n");
    APP_INFO0("    1: Single frequency\n");
    APP_INFO0("    3: Channel Map\n");
    APP_INFO0("    4: Single frequency with Tx Interval\n");
    tx_test_cmd.hopping_mode= app_get_choice("Enter the hopping mode (0 - 1, 3 - 4)");


    if (tx_test_cmd.hopping_mode == 3) {

        tx_test_cmd.frequency = 0;

        APP_INFO0("Channel Map:");
        APP_INFO0("    Channel map must be 10 bytes long, with each byte being separated by a space, and should not have a 0x prefix.");
        APP_INFO0("    e.g. 40 00 00 00 00 00 00 00 00 11, means enable CH0, CH4 and CH78.");
        APP_INFO0("    Enter the channel map:");
        if (scanf("%hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx %hhx",
                    &tx_test_cmd.chan_map[0], &tx_test_cmd.chan_map[1],
                    &tx_test_cmd.chan_map[2], &tx_test_cmd.chan_map[3],
                    &tx_test_cmd.chan_map[4], &tx_test_cmd.chan_map[5],
                    &tx_test_cmd.chan_map[6], &tx_test_cmd.chan_map[7],
                    &tx_test_cmd.chan_map[8], &tx_test_cmd.chan_map[9]) != 10)
        {
            APP_ERROR0("Channel map not entered correctly");
            return;
        }
        getchar();
    } else {
        APP_INFO0("Frequency:\n");
        APP_INFO0("    0: 2402 MHz\n");
        APP_INFO0("    78: 2480 MHz\n");
        tx_test_cmd.frequency = app_get_choice("Enter tx frequency (0 - 78)");

        if (tx_test_cmd.hopping_mode == 4) {
            APP_INFO0("\nConnection Interval (in frames):");
            tx_test_cmd.conn_interval = app_get_choice("Enter the connection interval(0 - 255):");

            APP_INFO0("\nNumber Of Packets in Group:");
            tx_test_cmd.num_of_packets = app_get_choice("Enter the number of packets in group(0 - 255):");
        }
    }

    APP_INFO0("Modulation_Type:\n");
    APP_INFO0("    1: 0x00  8-bit Pattern\n");
    APP_INFO0("    2: 0xFF  8-bit Pattern\n");
    APP_INFO0("    3: 0xAA  8-bit Pattern\n");
    APP_INFO0("    4: PRBS9 Pattern\n");
    APP_INFO0("    9: 0xF0  8-bit Pattern\n");
    tx_test_cmd.modulation_type= app_get_choice("Enter Modulation_Type (1 - 4, 9)");

    APP_INFO0("Logical_Channel:\n");
    APP_INFO0("    0: ACL EDR\n");
    APP_INFO0("    1: ACL Basic\n");
    APP_INFO0("    2: eSCO EDR\n");
    APP_INFO0("    3: eSCO Basic\n");
    APP_INFO0("    4: SCO Basic\n");
    tx_test_cmd.logical_channel = app_get_choice("Enter Logical_Channel (0 - 4)");

    APP_INFO0("BB_Packet_Type:\n");
    APP_INFO0("    0: NULL");
    APP_INFO0("    1: POLL");
    APP_INFO0("    2: FHS");
    APP_INFO0("    3: DM1");
    APP_INFO0("    4: DH1 / 2-DH1");
    APP_INFO0("    5: HV1");
    APP_INFO0("    6: HV2 / 2-EV3");
    APP_INFO0("    7: HV3 / EV3 / 3-EV3");
    APP_INFO0("    8: DV / 3-DH1");
    APP_INFO0("    9: AUX1 / PS");
    APP_INFO0("    10: DM3 / 2-DH3");
    APP_INFO0("    11: DH3 / 3-DH3");
    APP_INFO0("    12: EV4 / 2-EV5");
    APP_INFO0("    13: EV5 / 3-EV5");
    APP_INFO0("    14: DM5 / 2-DH5");
    APP_INFO0("    15: DH5 / 3-DH5");
    tx_test_cmd.bb_packet_type = app_get_choice("Enter BB_Packet_Type (0 - 15)");

    APP_INFO0("BB_Packet_Length:\n");
    tx_test_cmd.bb_packet_length = app_get_choice("Enter BB_Packet_Length (0 - 65535)");

    //APP_INFO0("Tx_Power_Level:\n");
    //APP_INFO0("    0: 0 dBm");
    //APP_INFO0("    1: -4 dBm");
    //APP_INFO0("    2: -8 dBm");
    //APP_INFO0("    3: -12 dBm");
    ...
    //APP_INFO0("    6: -24 dBm");
    //APP_INFO0("    7: -28 dBm");
    //APP_INFO0("    8: Specify Power in dBm");
    //APP_INFO0("    9: Specify Power Table index");
    //tx_test_cmd.tx_power_level = app_get_choice("Enter Tx_Power_Level (0 - 9)");
    tx_test_cmd.tx_power_level = 9;

    if (tx_test_cmd.tx_power_level == 8)
    {
        APP_INFO0("Transmit_Power_dBm:\n");
        tx_test_cmd.tx_power_dbm = app_get_choice("Enter Transmit_Power_dBm (-128 - 127)");
    }

    if (tx_test_cmd.tx_power_level == 9)
    {
        APP_INFO0("Transmit_Power_Table_Index:\n");
        tx_test_cmd.tx_power_index = app_get_choice("Enter Transmit_Power_Table_Index (0 - 7)");
        //tx_test_cmd.tx_power_index = 0;
    }
    ...
    BDADDR_TO_STREAM(p_data, tx_test_cmd.bd_addr);
    UINT8_TO_STREAM(p_data, tx_test_cmd.hopping_mode);
    UINT8_TO_STREAM(p_data, tx_test_cmd.frequency);
    if (tx_test_cmd.hopping_mode == 3) {
        REVERSE_ARRAY_TO_STREAM(p_data, tx_test_cmd.chan_map, BSA_TM_CHANNEL_MAP_LEN);
    }
    if (tx_test_cmd.hopping_mode == 4) {
        UINT8_TO_STREAM(p_data, tx_test_cmd.conn_interval);
        UINT8_TO_STREAM(p_data, tx_test_cmd.num_of_packets);
    }
    UINT8_TO_STREAM(p_data, tx_test_cmd.modulation_type);
    UINT8_TO_STREAM(p_data, tx_test_cmd.logical_channel);
    UINT8_TO_STREAM(p_data, tx_test_cmd.bb_packet_type);
    UINT16_TO_STREAM(p_data, tx_test_cmd.bb_packet_length);
    UINT8_TO_STREAM(p_data, tx_test_cmd.tx_power_level);
    UINT8_TO_STREAM(p_data, tx_test_cmd.tx_power_dbm);
    UINT8_TO_STREAM(p_data, tx_test_cmd.tx_power_index);
...
/*******************************************************************************
 **
 ** Function        app_tm_vsc_rx_test
 **
 ** Description     This function sends a RX_TEST VSC
 **
 ** Parameters      None
 **
 ** Returns         status: 0 if success / -1 otherwise
 **
 *******************************************************************************/
int app_tm_vsc_rx_test()
{
    tBSA_TM_VSC bsa_vsc;
    tBSA_STATUS bsa_status;
    UINT8 *p_data;
    tBSA_TM_RX_TEST_CMD rx_test_cmd;

    APP_INFO0("app_tm_vsc_rx_test");

    isTxTest = FALSE;

    memset(&rx_test_cmd,0,sizeof(tBSA_TM_RX_TEST_CMD));

    APP_INFO0("BR/EDR Rx test:\n");
    APP_INFO0("Enter the BD address of Remote Tx device(AA.BB.CC.DD.EE.FF):");

    if (scanf("%hhx.%hhx.%hhx.%hhx.%hhx.%hhx",
            &rx_test_cmd.bd_addr[0], &rx_test_cmd.bd_addr[1],
            &rx_test_cmd.bd_addr[2], &rx_test_cmd.bd_addr[3],
            &rx_test_cmd.bd_addr[4], &rx_test_cmd.bd_addr[5]) != 6)
    {
        APP_ERROR0("BD address not entered correctly");
        return;
    }
    ...
    APP_INFO0("Report_Period in ms:\n");
    rx_test_cmd.report_peroid = app_get_choice("Enter Report_Period (1000 - 5000)");

    APP_INFO0("Frequency:\n");
    APP_INFO0("    0: 2402 MHz\n");
    APP_INFO0("    78: 2480 MHz\n");
    rx_test_cmd.frequency = app_get_choice("Enter tx frequency (0 - 78)");
    ...
    APP_INFO0("    4: PRBS9 Pattern\n");
    APP_INFO0("    9: 0xF0  8-bit Pattern\n");
    rx_test_cmd.modulation_type = app_get_choice("Enter Modulation_Type (1 - 4, 9)");
    ...
    APP_INFO0("    3: eSCO Basic\n");
    APP_INFO0("    4: SCO Basic\n");
    rx_test_cmd.logical_channel = app_get_choice("Enter Logical_Channel (0 - 4)");
    ...
    APP_INFO0("    14: DM5 / 2-DH5");
    APP_INFO0("    15: DH5 / 3-DH5");
    rx_test_cmd.bb_packet_type = app_get_choice("Enter BB_Packet_Type (0 - 15)");

    APP_INFO0("BB_Packet_Length:\n");
    rx_test_cmd.bb_packet_length = app_get_choice("Enter BB_Packet_Length (0 - 65535)");
    ...
    BDADDR_TO_STREAM(p_data, rx_test_cmd.bd_addr);
    UINT16_TO_STREAM(p_data, rx_test_cmd.report_peroid);
    UINT8_TO_STREAM(p_data, rx_test_cmd.frequency);
    UINT8_TO_STREAM(p_data, rx_test_cmd.modulation_type);
    UINT8_TO_STREAM(p_data, rx_test_cmd.logical_channel);
    UINT8_TO_STREAM(p_data, rx_test_cmd.bb_packet_type);
    UINT16_TO_STREAM(p_data, rx_test_cmd.bb_packet_length);
