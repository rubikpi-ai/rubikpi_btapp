...
int app_tm_vsc_read_raw_rssi(BD_ADDR bda)
{
    ...
    status = BSA_TmReadRawRssi(&ReadRssiReq);
    if (status != BSA_SUCCESS)
    {
        APP_ERROR1("BSA_TmReadRawRssi failed status:%d", status);
        return -1;
    }

    APP_INFO0("\tStatus:\t\t\tSuccess");
    APP_INFO1("\tRSSI:\t\t\t%d (dB)", ReadRssiReq.rssi);
    APP_INFO1("\tConnection handle:\t0x%02X (%d)",
            ReadRssiReq.conn_handle, ReadRssiReq.conn_handle);
...
/*******************************************************************************
 **
 ** Function         app_tm_add_trace_text
 **
 ** Description      This function is used to add the trace to bsa server
 **
 ** Returns          int
 **
 *******************************************************************************/
int app_tm_add_trace_text(char *txt)
{
    tBSA_TM_SET_TRACE_LEVEL trace_txt;
    tBSA_STATUS status;

    BSA_TmSetTraceTextInit(&trace_txt);

    strncpy((char*)trace_txt.trace_text, txt, BSA_TM_TRACE_LEN_MAX-1);

    status = BSA_TmSetTraceText(&trace_txt);
    if (status != BSA_SUCCESS)
    {
        APP_ERROR1("BSA_TmSetTraceLevel failed status:%d", status);
        return -1;
