...
/*******************************************************************************
 **
 ** Function       app_tm_vse_decode_tx_pkt_count
 **
 ** Description    Decode VSE Tx packets count
 **
 ** Parameters     event: TM Event received
 **                p_data: TM Event's data
 **
 ...
 **
 *******************************************************************************/
int app_tm_vse_decode_pkt_count (tBSA_TM_VSE_MSG *p_vse)
{
    UINT8 *p_data = p_vse->data;
    UINT32 sync_timeout_cnt, hec_err_cnt, total_rev_pkts, good_pkts;
    UINT32 crc_err_pkt, total_rev_bits, good_bits, err_bits;

    /* Extract information from data */
    STREAM_TO_UINT32(sync_timeout_cnt, p_data);
    STREAM_TO_UINT32(hec_err_cnt, p_data);
    STREAM_TO_UINT32(total_rev_pkts, p_data);
    STREAM_TO_UINT32(good_pkts, p_data);
    STREAM_TO_UINT32(crc_err_pkt, p_data);
    STREAM_TO_UINT32(total_rev_bits, p_data);
    STREAM_TO_UINT32(good_bits, p_data);
    STREAM_TO_UINT32(err_bits, p_data);

    if (isTxTest) {
        APP_INFO1("    Tx packet count: %d", total_rev_pkts);
    } else {
        APP_INFO1("    Sync Timeout Count : %d", sync_timeout_cnt);
        APP_INFO1("    HEC Error Count : %d", hec_err_cnt);
        APP_INFO1("    Total Received Packets : %d", total_rev_pkts);
        APP_INFO1("    Good Packets : %d", good_pkts);
        APP_INFO1("    CRC Error Packets : %d", crc_err_pkt);
        APP_INFO1("    Total Received Bits : %d", total_rev_bits);
        APP_INFO1("    Good Bits : %d", good_bits);
        APP_INFO1("    Error Bits : %d", err_bits);
    }
...
/*******************************************************************************
 **
 ** Function        app_tm_vse_pkt_register
 **
 ...
 **
 *******************************************************************************/
int app_tm_vse_pkt_register(int sub_event, tAPP_TM_CUSTOM_CBACK p_callback)
{
    ...
    bsa_register.p_callback = p_callback;
...
/*******************************************************************************
 **
 ** Function       app_tm_vse_pkt_count_callback
 **
 ** Description    Packet count Callback
 **
 ** Parameters     event: TM Event received
 **                p_data: TM Event's data
 **
 ...
 **
 *******************************************************************************/
void app_tm_vse_pkt_count_callback (tBSA_TM_EVT event, tBSA_TM_MSG *p_data)
{
    BOOLEAN exit_cback= FALSE;
    ...
    {
    case BSA_TM_VSE_EVT:
        APP_INFO0("VSE received ");
        /* Decode VSE Parameters */
        exit_cback = app_tm_vse_decode_pkt_count(&p_data->vse);
        break;
