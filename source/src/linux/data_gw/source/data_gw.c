...
void app_dg_uipc_cback(BT_HDR *p_msg)
{
    ...
    {
#if defined (DISABLE_APP_DG_TRACE) && (DISABLE_APP_DG_TRACE == FALSE)
...
int app_dg_listen(void)
{
...
#else
#if (IOS_CARPLAY_INCLUDED == TRUE)
    int j;
    #define IOS_LISTEN_UUID_NUM    2
    const char svc_name[][32] = { "Wireless iAP2", "Wireless Carplay"};
    char szKeyName[][48] = {"00000000-DECA-FADE-DECA-DEAFDECACAFF", "2D8D2466-E14D-451C-88BC-7301ABEA291A"}; /*Wireless iAP*/
#else
...
#if (IOS_IAP_INCLUDED == TRUE) && (IOS_CARPLAY_INCLUDED == TRUE)
    for(j = 0; j < IOS_LISTEN_UUID_NUM; j++)
    {
#endif
    ...
    if (choice == BSA_SPP_SERVICE_ID)
    {
        ...
        if(choice)
        {
        ...
        #else
        #if (IOS_CARPLAY_INCLUDED == TRUE)
            strcpy(param.service_name, svc_name[j]);
        #else
            ...
            #if (IOS_CARPLAY_INCLUDED == TRUE)
            APP_INFO1("Creating DG connection: %s", szKeyName[j]);
            sscanf (szKeyName[j], "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
...
#if (IOS_IAP_INCLUDED == TRUE) && (IOS_CARPLAY_INCLUDED == TRUE)
    }// end of for(j = 0; j < IOS_LISTEN_UUID_NUM; j++)
#endif
