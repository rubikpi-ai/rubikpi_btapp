...
UINT8 g_u8_netif_read_thread_flag;
...
static void *app_pan_netif_read_thread(void *para);
...
static void *app_pan_netif_read_thread(void *para)
{
    ...
    p_msg = NULL;
    g_u8_netif_read_thread_flag = TRUE;
    while (g_u8_netif_read_thread_flag)
    {
        if (p_msg == NULL)
        {
        ...
        if (p_msg)
        {
            APP_DEBUG1("%s free buffer here!", __func__);
            GKI_freebuf(p_msg);
            p_msg = NULL;
        }
    }
    APP_DEBUG1("%s exit!", __func__);
}
...
int app_pan_start(void)
{
    ...
    pthread_create(&app_pan_cb.netif_thread, NULL, app_pan_netif_read_thread, NULL);
...
int app_pan_stop(void)
{
    ...
    g_u8_netif_read_thread_flag = FALSE;
    pthread_join(app_pan_cb.netif_thread, NULL);
