...
#ifndef DEMO_MODE
    APP_AG_MENU_SP_ACCEPT,
    APP_AG_MENU_SP_REFUSE,
#endif
    APP_AG_KEY_PLAY,
...
{
    printf("Bluetooth Application HFP AG menu:\n");
#ifndef DEMO_MODE
    printf("    %d  => Initialize\n", APP_AG_KEY_INIT);
...
#ifndef DEMO_MODE
    printf("    %d => Accept Request\n",  APP_AG_MENU_SP_ACCEPT);
    printf("    %d => Refuse Request\n",  APP_AG_MENU_SP_REFUSE);
#endif
...
int hfp_ag_menu_handle(void)
{
    ...
    app_mgt_custom_cback_update(app_ag_mgt_callback);
    ...
    do
    {
        ...
        case HELP_MENU:
            ...
            break;
        case APP_AG_KEY_INIT:
            app_ag_init();
            app_ag_start(BSA_SCO_ROUTE_DEFAULT);
            g_hfp_ag_init = 1;

            break;
        case APP_AG_KEY_DEINIT:
            app_ag_end();
